...
pub struct Lcf {
    ...
    /// Path to delinked objects generated by the `delink` subcommand.
    #[argp(option, short = 'd')]
    delinks_path: PathBuf,

    /// Path to build directory containing the compiled objects and the resulting linked binaries.
    #[argp(option, short = 'b')]
    build_path: PathBuf,
}
...
impl Lcf {
    ...
    fn write_sections_section(
        ...
        self.write_module_section(lcf, objects, config_dir, &config.main_module, ModuleKind::Arm9)?;
        for autoload in &config.autoloads {
            self.write_module_section(lcf, objects, config_dir, &autoload.module, ModuleKind::Autoload(autoload.kind))?;
        }
    ...
    fn write_module_section(
        ...
        for file in &delinks.files {
            let (file_path, _) = file.split_file_ext();
            let base_path = if file.complete { &self.build_path } else { &self.delinks_path };
